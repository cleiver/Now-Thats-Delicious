...
function loadPlaces(map) {
  ...
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    ...
    if (!places.length) {
      alert("No places found near you!");
      return;
    }

    // bounds são uma área ao redor do marcador, essa área será utilizada
    // para centralizar e aproximar (zoom) o mapa
...
function typeAhead(search) {
  ...
  searchInput.on("keyup", function (e) {
    // check for arrow up, down or enter
